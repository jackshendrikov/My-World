...
def del_list(request, list_id: int, list_slug: str) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        task_count_done = Task.objects.filter(task_list=task_list.id, checked=True).count()
        task_count_undone = Task.objects.filter(task_list=task_list.id, checked=False).count()
